...
class BlobStorage():
    def __init__(self, account_name, container_name, account_key) -> None:
        ...
        self.blob_service_client = BlobServiceClient.from_connection_string(self.params)
...
for file in files:
    if file == 'hired_employees.csv':
        hired_employees = pd.read_csv(blob_storage.get_sas_url(file), names = ['id', 'name', 'datetime', 'department_id', 'job_id'])
    if file == 'jobs.csv':
        jobs = pd.read_csv(blob_storage.get_sas_url(file), names = ['id', 'job_id'])
    if file == 'departments.csv':
        departments = pd.read_csv(blob_storage.get_sas_url(file), names = ['id', 'department_id'])
